...
def main(complete_in_path, filename, output_path, lc, strain):
    bands, b_array = read_bands(complete_in_path)
    ...
    with open(f"{output_path}/Hydrostatic_strain_bands_info.dat", "a+") as f:
        f.write(f"File: {filename}, lattice constant: {lc}, strain: {strain}\n")
...
if __name__ == "__main__":
    ...
    try:
        ...
        raise ValueError("Please provide the input path (location of lc_* folders) and output path.")

    folders = [name for name in os.listdir(in_path) if ("lc" in name and os.path.isdir(f"{in_path}/{name}"))]
    
    for fold in folders:

        bands_files = fnmatch.filter(os.listdir(f"{in_path}/{fold}"), "*bands*.hdf5")

        for file in bands_files:
            filename = file.split(".hdf5")[0]
            lc = filename.split("lc_")[1].split("_s")[0]
            strain = filename.split("_s_")[1]
            main(f"{in_path}/{fold}/{filename}", filename, output_path, lc, strain)
